...
export async function handleBranchCommand({ gh, owner, repo, issueNumber, comment }) {
  ...
  try {
    ...
    await archiveClosedPullRequestsForIssueBranch({
      gh,
      owner,
      repo,
      issueNumber,
...
export async function handleBranchDeleteCommand({ gh, owner, repo, issueNumber }) {
  ...
  await gh.createComment(
    ...
    [
      ...
      associatedPr ? "" : null,
      associatedPr ? `Closed associated pull request: ${associatedPr.number}` : null,
      "",
...
async function archiveClosedPullRequestsForIssueBranch({ gh, owner, repo, issueNumber, branchName }) {
  if (typeof gh.listPullRequests !== "function" || typeof gh.archivePullRequest !== "function") {
  ...
  for (const pr of closedPullRequests || []) {
    ...
    const pullRequestId = pullRequestGraphqlId(pr);
    if (!pullRequestId) continue;
    await gh.archivePullRequest(pullRequestId);
    archived += 1;
  }
  return { archived, skipped: false };
}

function pullRequestGraphqlId(pr) {
  return pr?.node_id || pr?.id || null;
}
...
function pullRequestMatchesIssue(pr, issueNumber, branchName) {
  if (branchName && pullRequestHeadRefName(pr) === branchName) return true;
  if (extractIssueNumberFromBranch(pullRequestHeadRefName(pr)) === issueNumber) return true;
  if (String(pr?.title || "").includes(`(#${issueNumber})`)) return true;
  if (bodyClosesIssue(pr?.body || "", issueNumber)) return true;
  return false;
}

function pullRequestHeadRefName(pr) {
  return pr?.head?.ref || pr?.headRefName || null;
}
...
async function listClosedPullRequestsForIssueBranch({ gh, owner, repo, issueNumber, branchName }) {
  const pullRequests = new Map();

  if (typeof gh.listPullRequests === "function") {
    const closedByHead = await gh.listPullRequests(owner, repo, {
      state: "closed",
      head: `${owner}:${branchName}`,
      sort: "updated",
      direction: "desc",
...
function removePullRequestIssueLinks(body, issueNumber, branchName, { emptyFallback = "This PR was archived by automation." } = {}) {
  ...
  const cleanedBody = originalBody
    .split(/\r?\n/)
    .filter((line) => {
      if (bodyClosesIssue(line, issueNumber) || lineReferencesIssue(line, escaped)) {
        changed = true;
        return false;
      }
      if (isPullRequestBranchLinkLine(line, branchName)) {
        changed = true;
        return false;
      }
      return true;
...
function lineReferencesIssue(line, escapedIssueNumber) {
  ...
  return new RegExp(`(^|\\s)(?:[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+)?#${escapedIssueNumber}\\b`, "i").test(text)
    || new RegExp(`https://github\\.com/[^\\s)]+/[^\\s)]+/issues/${escapedIssueNumber}\\b`, "i").test(text);
}

function isPullRequestBranchLinkLine(line, branchName) {
  const trimmed = String(line || "").trim();
  if (!trimmed.startsWith("Branch:")) return false;
  if (!branchName) return /^Branch:\s+\[`[^`]+`\]\(https:\/\/github\.com\/[^)]+\)\s*$/i.test(trimmed);
  const escapedBranchName = String(branchName).replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  return new RegExp(`^Branch:\\s+\\[\`${escapedBranchName}\`\\]\\(https://github\\.com/[^)]+\\)\\s*$`, "i").test(trimmed);
}
